...
export type WritingCopy = {
  ...
  postsInYear: string;
  latestInYear: string;
  yearPageLead: (year: string, count: number) => string;
  byMonth: string;
  monthNames: string[];
  allTags: string;
...
export const writingCopy: Record<Lang, WritingCopy> = {
  'zh-CN': {
    ...
    postsInYear: '篇',
    latestInYear: '最近更新',
    yearPageLead: (year, count) =>
      `${year} 年共 ${count} 篇，按月份排列。正式阅读与订阅在 blog.alexander.xin。`,
    byMonth: '按月',
    monthNames: [
      '1 月',
      '2 月',
  ...
  'zh-TW': {
    ...
    latestInYear: '最近更新',
    yearPageLead: (year, count) =>
      `${year} 年共 ${count} 篇，按月份排列。正式閱讀與訂閱在 blog.alexander.xin。`,
    byMonth: '按月',
    monthNames: [
      '1 月',
      '2 月',
  ...
  'en-GB': {
    ...
    latestInYear: 'Latest',
    yearPageLead: (year, count) =>
      `${count} ${count === 1 ? 'post' : 'posts'} from ${year}, grouped by month. Canonical reading on blog.alexander.xin.`,
    byMonth: 'By month',
    monthNames: [
      'January',
      'February',
  ...
  fr: {
    ...
    latestInYear: 'Dernière',
    yearPageLead: (year, count) =>
      `${count} ${count === 1 ? 'article' : 'articles'} en ${year}, groupés par mois. Lecture canonique sur blog.alexander.xin.`,
    byMonth: 'Par mois',
    monthNames: [
      'Janvier',
      'Février',
  ...
  ru: {
    ...
    yearLabel: '',
    postsInYear: 'записей',
    latestInYear: 'Последнее',
    yearPageLead: (year, count) =>
      `${count} ${count === 1 ? 'запись' : 'записей'} за ${year}, по месяцам. Каноническое чтение — на blog.alexander.xin.`,
    byMonth: 'По месяцам',
    monthNames: [
      'Январь',
      'Февраль',
