/**
 * Upload legacy-redirect Worker (index.js + security-headers.js).
...
const src = join(root, '../workers/legacy-redirect/src')
const index = readFileSync(join(src, 'index.js'), 'utf8')
const headers = readFileSync(join(src, 'security-headers.js'), 'utf8')
...
const body = [
  ...
  index,
  `--${boundary}`,
  'Content-Disposition: form-data; name="security-headers.js"; filename="security-headers.js"',
  'Content-Type: application/javascript+module',
  '',
  headers,
  `--${boundary}--`,
