...
// 优化页面加载器移除函数
function initPageLoaderRemoval() {
            ...
            entries.forEach((entry) => {
                if (entry.isIntersecting) {
                    ...
                    img.onerror = function () {
                        ...
                        this.classList.add('img-error');
                        console.log(
                            `Lazy image failed to load: ${this.getAttribute(
                                'data-src'
                            )}`
                        );
                    };
