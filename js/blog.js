...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateArticleCount() {
        const visiblePosts = Array.from(blogPosts).filter(
            (post) => post.style.display !== 'none'
        );
        articleCount.textContent = visiblePosts.length;
    }
    ...
    function sortBlogPosts() {
        ...
        switch (sortBy) {
            ...
            case 'popular':
                // 按阅读量排序（新增）
                visiblePosts.sort((a, b) => {
                    const viewsA = getPostViews(a);
                    const viewsB = getPostViews(b);
    ...
    function getPostViews(post) {
        const viewsText = post.querySelector(
            '.blog-card-meta span:nth-child(2)'
        ).textContent;
        return parseInt(viewsText.match(/\d+/)[0]);
    }
